...
class dialogUi(QDialog):
    def __init__(self, parent=None):
        ...
        self.reply_east = False
        self.reply_west = False
        ...
        self.d.bn_east.clicked.connect(lambda: self.diag_return_east())
        self.d.bn_west.clicked.connect(lambda: self.diag_return_west())
        ##############################################################################################
    ...
    def mousePressEvent(self, event):
        self.dragPos = event.globalPos()
    #################################################################################################
    def diag_return_east(self):
        self.reply_east = True
        self.close()

    def diag_return_west(self):
        self.reply_west = True
        self.close()
    #################################################################################################                        ------(C3)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ui.bn_text.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_text'))
        self.ui.bn_voice.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_voice'))
        self.ui.bn_noise.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_noise'))
        self.ui.bn_image.clicked.connect(lambda: UIFunction.buttonPressed(self, 'bn_image'))
        #############################################################
        ...
        #THIS CODE INITIALISED THE COMBOBOX, SETTING THE ENABLE SIGNAL TO ZERO, SO THAT THE ROLE CANNOT BE SELECTED UNTIL WE LOAD THE MODEL.         ---------(C11)
        UIFunction.initComboBox(self)
        #############################################################

        # ----> BUTTON PRESSED EVENTSZHIWE WHICH SET THE ENABLE SIGNAL OF THE COMBOBOX
        # THE FUNCTION OF THIS CODE IS TO ENABLE THE COMBOBOX SELECTED BY THE CHARACTER TO BE FREED AFTER CLICKING TO LOAD THE MODEL.          ---------(C12)
        self.ui.TTS_pushButton_StartLoading.clicked.connect(lambda: UIFunction.enableComboBox(self, self.ui.TTS_comboBox_SelectRole))
        self.ui.VC_pushButton_StartLoading.clicked.connect(lambda: UIFunction.enableComboBox(self, self.ui.VC_comboBox_SelectRole))
        #############################################################
...
#FOR REST OF THE WIDGET GOTO THE Documentation.pdf AND CHECK THE LAYOUT FOR MORE DETAILS.
    ...
    def errorexec(self, heading, icon, btnOk):
        ...
        self.error.exec_()
    # #############################################################
    # ALWAYS ADD THE FOLLOWING CODE IN THE ui_main.py UI_MAINWINDOW CLASS setupUI FUNCTION TO ENSURE THE DISPLAY OF FONT.
